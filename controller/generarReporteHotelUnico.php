<?php
   ...
   //set header and footer to the pdf
    ...
    $texto = "ERROR: ";
        ...
        //$mpdf->WriteHTML($consulta);
        if ($resultado = $mysqli->query($consulta)) {

            while($fila = $resultado->fetch_row()){
                $mpdf->WriteHTML('<p style="text-align:center; "><b>Reporte de Recolección '.$fila[1].'</b></p>');
                ...
                $sql_reserva = "SELECT r.id_reserva, r.id_cliente, r.habitacion, r.num_adultos, r.num_ninos, r.num_insen, r.id_tour, r.horario FROM reserva r WHERE r.clave_institucion='".$fila[0]."' AND r.fecha ='".$fecha."' ORDER BY r.id_tour ASC";
                ...
                if ($res_reserva = $mysqli->query($sql_reserva)) {
                    ...
                    if($row_cnt != 0){
                        ////////////////////////////////
                        $sql_num = "SELECT SUM(r.num_adultos), SUM(r.num_ninos), SUM(r.num_insen) FROM reserva r WHERE r.clave_institucion='".$fila[0]."' AND r.fecha ='".$fecha."'";
